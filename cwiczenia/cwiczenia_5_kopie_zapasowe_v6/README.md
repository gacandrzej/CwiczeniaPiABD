...
1. Stwórz dwie bazy w phpMyAdmin z dwiema tabelami, po 4 kolumny, 2
    wiersze danych.
1. W shellu utwórz trzecią bazę danych o nazwie biblioteka z jedną
...
1. Nadać uprawnienia do powyższych baz dla wybranych użytkowników.(po
    punkcie 5 sprawdzić, czy w plikach kopii znajdują się instrukcje
    `GRANT`)

1. Wykonaj eksport baz danych:
...
1. Wyeksportuj wybraną tabelę w shellu z użyciem SELECT
   kolumny from _table_ `INTO OUTFILE` _plik_.

   ![image7](media/image7.png)

   ![image8](media/image8.png)

1. Zaimportuj tabelę z pomocą `LOAD DATA INFILE` _plik_ `INTO TABLE` _tabela._  
...
1. Zaimportuj dwie bazy XML i JSON.ZIP w _*phpMyAdmin*_.
1. Zaimportuj bazę danych wykonaną w punkcie 4b w shellu z użyciem
    `SOURCE`.

1. Sprawdź poprawność przywrócenia w phpMyAdmin i w shellu.

1. Utwórz dwóch operatorów kopii zapasowych, którzy mają nadane
    uprawnienia do wykonywania kopii o nazwach **admin** i **admin2**.
    Drugi z operatorów może także przywracać kopie.

1. Utwórz skrypt dla narzędzi mysqldump.exe oraz mariadb-dump,
   który wykona kopię 4 baz. Sprawdź jego działanie.

1. Zmodyfikuj skrypt tak, aby w nazwie pliku kopii pojawiała się data i
    czas utworzenia.
...
1. Utwórz skrypt i z pomocą harmonogramu w Microsoft SQL Server
...
1. Stwórz bazę w PgAdmin, następnie wykonaj jej kopię. Usuń bazę, a
    następnie odtwórz dane z kopii.

1. Stwórz z pomocą **harmonogramu** i skryptu kopię bazy w postgreSQL,
    zaplanuj jej cykliczność o zadanych godzinach i dniach.

1. Sprawdź poprawność przywrócenia.

1. Na jednym z komputerów udostępnić w systemie linux bazę o nazwie
